...
public class MenuSummaryController
   ...
   /**
    * Wenn der Benutzer nichts gewählt hat, wird in das in der JTable angezeigt.
    */
    ...
    * @param lStore Store.
    */
   public MenuSummaryController(NodeStore lStore) {
	store = lStore;
   ...
   /**
    * Liest die eingegebenen Parameter der vorherigen Panels aus.
    *	Wandelt die Parameter (nur für die Anzeige) Boolean von True und False in Yes und No, sowie 
    * leere Parameter in --------- ;
    * 
   ...
   public void readSettings(WizardDescriptor lWiz) {
	...
	while (nodeIterator.hasNext()) {
	   ...
	   while (configItemIterator.hasNext()) {
		...
		} else if (store.getNode(nodeId).getConfigItem(configItemName).getConfigType().equals(Types.Boolean)) {
