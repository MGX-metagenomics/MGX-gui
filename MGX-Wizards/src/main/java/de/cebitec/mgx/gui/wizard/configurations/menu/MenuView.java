...
public final class MenuView extends JPanel implements DocumentListener {
   ...
   /**
    * HashMap um zum passenden ConfigItem den passenden JFileChooser aufzurufen.
    */
    ...
    * @param docListener
    */
   protected MenuView(int lFieldCounter, String lDisplayName, ArrayList<String> lUserNames, ArrayList<String> lUserDescriptions, ArrayList<String> lConfigTypes, ArrayList<ArrayList<String>> lUserChoicesValues, ArrayList<ArrayList<String>> lUserChoicesDescriptions, ArrayList<String> lDefaultValues, int lMandatoryComponentsCounter, ActionListener lListener, DocumentListener ldocListener) {
	...
	setUpPanelForm(lConfigTypes, lUserDescriptions, lUserNames, lUserChoicesDescriptions, lUserChoicesValues, lMandatoryComponentsCounter, lFieldCounter);
    ...
    */
   private int assemblePanels(JPanel lComponentDescriptionPanel, ArrayList<String> lUserNames, int configIndex, JPanel lDescriptionPanel, JLabel lDescriptionLabel, int lStartLabel, GridBagConstraints lComponentConstraints, JPanel lComponentPanel, int lMandatoryComponentsCounter, GridBagConstraints lAllInOneConstraints, int lAddToConfigIndex, JPanel lRightComponentPanel, JLabel lOptionalLabel, int lFieldCounter) {
	Font font = new Font(Font.DIALOG, Font.BOLD, 14);
	lComponentDescriptionPanel.setBorder(
	    BorderFactory.createTitledBorder(null, lUserNames.get(configIndex), TitledBorder.CENTER, TitledBorder.DEFAULT_JUSTIFICATION, font, Color.BLACK));
	...
	if (configIndex != 0 && configIndex == lMandatoryComponentsCounter && lMandatoryComponentsCounter != lUserNames.size()) {
	...
	JLabel numberLabel = new JLabel(lFieldCounter + "." + Integer.toString(configIndex + 1));
    ...
    */
   private void assemblePanelsAndScrollPane(JPanel lAllInOnePanel, JPanel lRightComponentPanel) {
    ...
    * @param docListener
    */
   private void setUpPanelForm(ArrayList<String> lConfigTypes, ArrayList<String> lUserDescriptions, ArrayList<String> lUserNames, ArrayList<ArrayList<String>> lUserChoiceDescriptions, ArrayList<ArrayList<String>> lUserChoiceValues, int lMandatoryComponentsCounter, int lFieldCounter) {
	...
	allInOnePanel.setLayout(new BoxLayout(allInOnePanel, BoxLayout.PAGE_AXIS));
	...
	JLabel mandatoryLabel = new JLabel("<html><u>Mandatory Fields:</u></html>");
	JLabel optionalLabel = new JLabel("<html><u>Optional Fields:</u></html>");
	...
	if (lMandatoryComponentsCounter == lUserNames.size() || lMandatoryComponentsCounter != 0) {
	...
	for (int configIndex = 0; configIndex < lUserNames.size(); configIndex++) {
	   ...
	   JLabel descriptionLabel = new JLabel(initialText + "<html><table><td width=400>" + lUserDescriptions.get(configIndex) + "</td></table></html>");



	   if (lConfigTypes.get(configIndex).equals(Types.Boolean)) {

		initializeRadioButtons(componentPanel, configIndex, lMandatoryComponentsCounter, defaultValue);
	   ...
	   } else if (lConfigTypes.get(configIndex).equals(Types.Selection) || lConfigTypes.get(configIndex).equals(Types.Enumeration)) {

		initializeComboBox(lUserChoiceValues, choiceIndex, lUserChoiceDescriptions, defaultValue, choicesCounter, componentPanel, orderFont);

		choiceIndex++;
		startLabel = 0;

	   } else {
		initializeTextFieldsAreas(lConfigTypes, configIndex, orderFont, componentPanel, defaultValue);
		startLabel = 0;
	   }
	   startLabel = assemblePanels(componentDescriptionPanel, lUserNames, configIndex, descriptionPanel, descriptionLabel, startLabel, componentConstraints, componentPanel, lMandatoryComponentsCounter, allInOneConstraints, addToConfigIndex, rightComponentPanel, optionalLabel, lFieldCounter);
	}
    ...
    */
   private void initializeFileChooser(JPanel lComponentPanel, Font lMessageFont, int lConfigIndex) {
	...
	openButton.setActionCommand(ActionCommands.FileOpen + ":" + lConfigIndex);
	openButton.addActionListener(listener);
	removeButton.addActionListener(listener);
	removeButton.setActionCommand(ActionCommands.FileClear + ":" + lConfigIndex);
	lComponentPanel.add(openButton);

	lComponentPanel.add(fileField);
	lComponentPanel.add(removeButton);

	messageLabel = new JLabel("<html><table><td width=400>" + Messages.getInformation(Messages.File) + "</td></table></html>");
    ...
    */
   private void initializeTextFieldsAreas(ArrayList<String> lConfigTypes, int lConfigIndex, Font lMessageFont, JPanel lComponentPanel, String lDefaultValue) {

	JButton defaultButton;
	if (lConfigTypes.get(lConfigIndex).equals(Types.Byte)) {
	   messageLabel = new JLabel("<html><table><td width=400>" + Messages.getInformation(Messages.Byte) + "</td></table></html>");

	} else if (lConfigTypes.get(lConfigIndex).equals(Types.Double)) {
	   messageLabel = new JLabel("<html><table><td width=400>" + Messages.getInformation(Messages.Double) + "</td></table></html>");

	} else if (lConfigTypes.get(lConfigIndex).equals(Types.Integer)) {
	   messageLabel = new JLabel("<html><table><td width=400>" + Messages.getInformation(Messages.Integer) + "</td></table></html>");

	} else if (lConfigTypes.get(lConfigIndex).equals(Types.Long)) {
	   messageLabel = new JLabel("<html><table><td width=400>" + Messages.getInformation(Messages.Long) + "</td></table></html>");

	} else if (lConfigTypes.get(lConfigIndex).equals(Types.ULong)) {
	   messageLabel = new JLabel("<html><table><td width=400>" + Messages.getInformation(Messages.ULong) + "</td></table></html>");

	} else if (lConfigTypes.get(lConfigIndex).equals(Types.SByte)) {
	   messageLabel = new JLabel("<html><table><td width=400>" + Messages.getInformation(Messages.SByte) + "</td></table></html>");

	} else if (lConfigTypes.get(lConfigIndex).equals(Types.String)) {
	   messageLabel = new JLabel("<html><table><td width=400>" + Messages.getInformation(Messages.String) + "</td></table></html>");
	}
	messageLabel.setFont(lMessageFont);
	if (lConfigTypes.get(lConfigIndex).equals(Types.String)) {
	   ...
	   pane.setSize(new Dimension(460, 130));
	   pane.setPreferredSize(new Dimension(460, 130));
	   pane.setMinimumSize(new Dimension(460, 130));
	   pane.setMaximumSize(new Dimension(460, 130));
	   ...
	   if (lDefaultValue != null) {
		...
		defaultButton.setActionCommand(ActionCommands.Default + ":" + Integer.toString(lConfigIndex));
    ...
    * ComboBox.
    */
   private void initializeComboBox(ArrayList<ArrayList<String>> lUserChoiceValues, int lChoiceIndex, ArrayList<ArrayList<String>> lUserChoiceDescriptions, String lDefaultValue, int lChoicesCounter, JPanel lComponentPanel, Font lMessageFont) {
	JComboBox comboBox;
	...
	for (int j = 0; j < lUserChoiceValues.get(lChoiceIndex).size(); j++) {
	   ...
	   if (lDefaultValue != null) {
		...
		if (choiceValue.equals(lDefaultValue)) {
		   selectedIndex = j + 1;
		   choicesValues[selectedIndex] = choicesValues[selectedIndex] + " (Default)";
		}
	   } else {
		selectedIndex = 0;
	   }
	...
	messageLabel = new JLabel("<html><table><td width=400>" + Messages.getInformation(Messages.Enumeration) + "</td></table></html>");
	messageLabel.setFont(lMessageFont);
   }
    ...
    */
   private void initializeRadioButtons(JPanel lComponentPanel, int lConfigIndex, int lMandatoryComponentsCounter, String lDefaultValue) {
   ...
   protected void chooseFile(int lIndex) {
	...
	if (allComponentsList.get(lIndex) instanceof JTextField) {
	   ...
	   if (returnVal == JFileChooser.APPROVE_OPTION) {
		...
		((JTextField) allComponentsList.get(lIndex)).setText(file.getPath() + "." + "\n");
		((JTextField) allComponentsList.get(lIndex)).setToolTipText(file.getPath());
   ...
   protected void setDefault(int lIndex) {
	if (allComponentsList.get(lIndex) instanceof JTextField) {
	   ((JTextField) allComponentsList.get(lIndex)).setText(defaultValues.get(lIndex));
	} else if (allComponentsList.get(lIndex) instanceof JTextArea) {
	   ((JTextArea) allComponentsList.get(lIndex)).setText(defaultValues.get(lIndex));
	} else if (allComponentsList.get(lIndex) instanceof ButtonGroup) {

	   ((ButtonGroup) allComponentsList.get(lIndex)).clearSelection();
	   
	   if (defaultValues.get(lIndex).equalsIgnoreCase(ActionCommands.RadioTrue)) {

		radioButtons.get(lIndex).get(0).setSelected(true);

	   } else if (defaultValues.get(lIndex).equalsIgnoreCase(ActionCommands.RadioFalse)) {

		radioButtons.get(lIndex).get(1).setSelected(true);

	   } 
	} else if(allComponentsList.get(lIndex) instanceof JComboBox){
	   ((JComboBox) allComponentsList.get(lIndex)).setSelectedItem(defaultValues.get(lIndex));
	}
   }
}
